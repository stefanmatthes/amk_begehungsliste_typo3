...
/***
 *
 * This file is part of the "Laufliste" Extension for TYPO3 CMS.tx_smlaufliste_domain_model_problem LEFT JOIN fe_user on fe.user.uid =  where modul =".$modulid.
 *
...
/**
 * The repository for Problems
 */
class ProblemRepository extends \TYPO3\CMS\Extbase\Persistence\Repository
{
    /**
...
',
     ...
     * @param $modulid
     */
    public function findByModulID($modulid)
    {
        //    echo "Modul auslesen mit TokenID: ".$tokenid;
        $query = $this->createQuery();
        $query->getQuerySettings()->setRespectStoragePage(FALSE);
        $query->matching($query->logicalAnd($query->equals('modul', $modulid), $query->equals('hidden', 0), $query->equals('deleted', 0)));
        //   $query->statement('select p.*,f.first_name,f.last_name from tx_smlaufliste_domain_model_problem p   LEFT JOIN fe_users f on f.uid = p.feuser where p.deleted=0 and p.hidden=0 and modul =' . $modulid . '');
    ...
    /**
     * Find Offenen Störungen
     *
     * @param $pid
     */
    public function findAllOpen($pid)
    {
        //    echo "Modul auslesen mit TokenID: ".$tokenid;
        $query = $this->createQuery();
        $query->getQuerySettings()->setRespectStoragePage(FALSE);
        $query->matching($query->logicalAnd($query->equals('pid', $pid), $query->equals('status', 0), $query->equals('hidden', 0), $query->equals('deleted', 0)));
        //   $query->statement('select p.*,f.first_name,f.last_name from tx_smlaufliste_domain_model_problem p   LEFT JOIN fe_users f on f.uid = p.feuser where p.deleted=0 and p.hidden=0 and modul =' . $modulid . '');
        $result = $query->execute();
        //  $this->debugQuery($query);
        //     krexx($result);
    ...
    /**
     * Find Offenen Störungen  by ModulID
     *
     * @param $modulid
     */
    public function findOpenByModulID($modulid)
    {
        //    echo "Modul auslesen mit TokenID: ".$tokenid;
        $query = $this->createQuery();
        $query->getQuerySettings()->setRespectStoragePage(FALSE);
        $query->matching($query->logicalAnd($query->equals('modul', $modulid), $query->equals('status', 0), $query->equals('hidden', 0), $query->equals('deleted', 0)));
        //   $query->statement('select p.*,f.first_name,f.last_name from tx_smlaufliste_domain_model_problem p   LEFT JOIN fe_users f on f.uid = p.feuser where p.deleted=0 and p.hidden=0 and modul =' . $modulid . '');
        $result = $query->execute();
        //     krexx($result);
        //   $query->setOrderings($this->orderByField('uid', $uidArray));
