...
/**
 * The repository for Rundgangaktivs
 */
class RundgangaktivRepository extends \TYPO3\CMS\Extbase\Persistence\Repository
{
    /**
...
',
     ...
     * @param $rundgangUid
     */
    public function findAktiverRundgang($schichtUid, $rundgangUid)
    {
        ...
        $query->matching($query->logicalAnd($query->equals('schicht', $schichtUid), $query->equals('rundgang', $rundgangUid), $query->greaterThanOrEqual('zeitstart', $date->modify('-12 hours')), $query->equals('hidden', 0), $query->equals('deleted', 0)));
        // finde den letzten offenen Rundgang
        //    $query->matching($query->logicalAnd($query->equals('schicht', $schichtUid), $query->equals('rundgang', $rundgangUid),$query->equals('hidden', 0), $query->equals('deleted', 0)));
     ...
     * @param $rundgangUid
     */
    public function findByRundgangUid($rundgangUid)
    {
        //    echo "Modul auslesen mit TokenID: ".$tokenid;
        $query = $this->createQuery();
        //$date = new \DateTime('midnight');
     //   $date = new \DateTime();
        $query->getQuerySettings()->setRespectStoragePage(FALSE);
      //  $query->setOrderings(['uid' => \TYPO3\CMS\Extbase\Persistence\QueryInterface::ORDER_DESCENDING]);
        $query->matching($query->logicalAnd( $query->equals('uid', $rundgangUid)));
        // finde den letzten offenen Rundgang
        //    $query->matching($query->logicalAnd($query->equals('schicht', $schichtUid), $query->equals('rundgang', $rundgangUid),$query->equals('hidden', 0), $query->equals('deleted', 0)));
      //   $this->debugQuery($query);
     ...
     * @param $rundgangUid
     */
    public function findOffeneAktiveRundgaenge($rundgangUid)
    {
        //    echo "Modul auslesen mit TokenID: ".$tokenid;
        $query = $this->createQuery();
        //$date = new \DateTime('midnight');
        $query->getQuerySettings()->setRespectStoragePage(FALSE);
        //$query->setOrderings(['uid' => \TYPO3\CMS\Extbase\Persistence\QueryInterface::ORDER_DESCENDING]);
        $query->matching($query->logicalAnd($query->equals('rundgang', $rundgangUid), $query->equals('hidden', 0), $query->equals('deleted', 0)));
        // finde den letzten offenen Rundgang
        //    $query->matching($query->logicalAnd($query->equals('schicht', $schichtUid), $query->equals('rundgang', $rundgangUid),$query->equals('hidden', 0), $query->equals('deleted', 0)));
        //$this->debugQuery($query);
        return $query->execute();
